...
import AdminGovernanceBranding from "@/components/admin/governance/AdminGovernanceBranding";
...
export default function AdminGovernanceChanges() {
  ...
  const { data: contributions = [], isLoading, error } = useGovernanceContributions(
    null,
    !!user?.id && !!adminState?.is_admin,
  );
      ...
      <main className="mx-auto w-full max-w-5xl space-y-8 px-4 py-6 sm:px-6 sm:py-8">
        <section>
          <p className="max-w-2xl text-sm text-muted-foreground">
            Review proposed changes to the public governance model.
          </p>
        </section>

        <AdminGovernanceBranding />

        <section className="space-y-3">
          <div>
            <h2 className="text-lg font-semibold">Pending changes</h2>
            <p className="text-sm text-muted-foreground">
              Review proposed additions, corrections, moves, and removals.
            </p>
          </div>
