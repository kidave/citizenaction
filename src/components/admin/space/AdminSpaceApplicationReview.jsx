          ...
          <Card>
            <CardHeader>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <CardTitle className="text-2xl">{application.proposed_name}</CardTitle>
                  <p className="mt-1 text-sm text-muted-foreground">Application #{application.id.slice(0, 8).toUpperCase()}</p>
                </div>
            ...
            <CardContent className="space-y-8">
              <section className="grid gap-6 sm:grid-cols-2">
                <Info label="Requested URL" value={`/${application.proposed_slug}`} />
                <Info label="Contact email" value={application.email} />
                <Info label="Contact number" value={application.contact_number} />
                <Info label="Website" value={application.website} />
                <Info label="Address" value={application.address} />
                <Info label="Applicant category" value={application.category} />
                <div className="sm:col-span-2"><Info label="Description" value={application.description} /></div>
                <div className="sm:col-span-2"><Info label="Why should this Space exist?" value={application.justification} /></div>
              </section>
                ...
                <>
                  ...
                  <section className="space-y-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Select the Citizen Action category that best represents this Space.</p>
                    </div>
                  ...
                  <section className="space-y-2">
                    ...
                    <Textarea value={adminNotes} onChange={(event) => setAdminNotes(event.target.value)} placeholder="Add review notes..." rows={5} disabled={saving} />
                  </section>

                  <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                    <Button variant="destructive" onClick={() => review("reject")} disabled={saving}><XCircle className="mr-2 h-4 w-4" />Reject application</Button>
                    <Button onClick={() => review("approve")} disabled={saving || !categoryId}><CheckCircle2 className="mr-2 h-4 w-4" />Approve application</Button>
                  </div>
                </>
              )}
            </CardContent>
