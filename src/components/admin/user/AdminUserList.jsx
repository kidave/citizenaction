      ...
      <main className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Manage registered users and platform administrator roles.
          </p>
        </div>
              ...
              <div className="divide-y">
                {users.map((user) => {
                  ...
                  return (
                    <div
                      key={user.user_id}
                      className="flex flex-col gap-4 px-4 py-4 sm:flex-row sm:items-center sm:px-6"
                    >
                      <div className="flex min-w-0 flex-1 items-center gap-3">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full border bg-muted">
                          {user.avatar_url ? (
                            ...
                            <span className="text-sm font-medium">{initials(user.name)}</span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-3 sm:shrink-0">
                        ...
                        <Select
                          ...
                          onValueChange={(role) => changeRole(user.user_id, role)}
                          disabled={busy}
                        >
