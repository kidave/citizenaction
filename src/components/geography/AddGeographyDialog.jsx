...
async function fetchGeographyAncestors(item) {
  ...
  while (parentId && ancestors.length < 20) {
    const { data, error } = await supabase
      .from("geographies")
      .select("id,name,official_name,geography_type,parent_id,country_code,osm_type,osm_id,admin_level,center,metadata")
      .eq("id", parentId)
      .maybeSingle();
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}>
    <SheetContent side="right" className="flex w-full flex-col gap-0 p-0 sm:max-w-3xl md:max-w-5xl">
      ...
      <div className="grid min-h-0 flex-1 md:grid-cols-[360px_1fr]">
        <div className="flex min-h-0 flex-col border-r">
          <div className="space-y-3 border-b p-4">
            ...
            {path.length > 0 && !search && <div className="flex items-center gap-1 overflow-hidden text-xs text-muted-foreground">
              <button type="button" onClick={() => { setParent(null); setPath([]); setSearch(""); }} className="shrink-0 hover:text-foreground">India</button>
              {path.slice(1).map((item) => <span key={item.id} className="flex min-w-0 items-center gap-1"><ChevronRight className="h-3 w-3 shrink-0" /><span className="truncate">{geographyLabel(item)}</span></span>)}
            </div>}
          </div>
              ...
              {isLoading ? <div className="flex items-center gap-2 px-3 py-8 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Loading boundaries...</div> : items.length === 0 ? <div className="px-3 py-8 text-center text-sm text-muted-foreground">No matching boundaries found.</div> : items.map((item) => {
                ...
                return <div key={item.id} className={`flex items-center gap-2 rounded-lg px-2 py-1 ${isSelected ? "bg-accent" : ""}`}>
                  <label htmlFor={`geography-${item.id}`} className="flex min-w-0 flex-1 cursor-pointer items-center gap-3 rounded-md px-2 py-2 text-left hover:bg-muted">
                    <RadioGroupItem id={`geography-${item.id}`} value={item.id} onClick={() => { if (search) handleSearchSelection(item); else selectGeography(item); }} className="shrink-0" />
                  ...
                  {browsing && <Button type="button" size="icon" variant="ghost" className="h-8 w-8 shrink-0" onClick={() => openChildren(item)} title={`Browse inside ${geographyLabel(item)}`} aria-label={`Browse inside ${geographyLabel(item)}`}><ChevronRight className="h-4 w-4" /></Button>}
                </div>;
              })}
            </RadioGroup>
          </ScrollArea>
        ...
        <div className="flex min-h-0 flex-col">
          <div className="relative min-h-[22rem] flex-1 bg-muted/20">
            <LeafletMap lat={Number(center?.lat) || 20.5937} lng={Number(center?.lng) || 78.9629} boundaries={mapBoundary} selectedBoundaryId={selected?.osm_id || null} showMarker={false} zoom={selected?.admin_level <= 4 ? 6 : selected?.admin_level === 5 ? 9 : 11} onChange={() => {}} onBoundaryClick={() => {}} />
      ...
      <SheetFooter className="border-t bg-background px-5 py-4 sm:px-6">
        {selected ? <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="truncate text-sm font-semibold">{geographyLabel(selected)}</p><p className="text-xs text-muted-foreground">{geographyTypeLabel(selected)}</p></div><div className="flex shrink-0 gap-2"><Button type="button" variant="outline" onClick={() => onOpenChange?.(false)} disabled={isSetting}>Cancel</Button><Button type="button" onClick={handleSave} disabled={isSetting || selected.id === currentGeographyId || loadingGeometry}>{isSetting ? "Saving..." : submitLabel}</Button></div></div> : <div className="w-full text-xs text-muted-foreground">India is the default starting point. Browse down through the administrative hierarchy or search for a boundary directly.</div>}
      </SheetFooter>
    </SheetContent>
  </Sheet>;
}
