...
export default function AddGeographyDialog({ open, onOpenChange, governanceId, entityName, onSaved }) {
  ...
  const mapBoundary = selected && geometry
    ? [{ osm_type: selected.osm_type, osm_id: selected.osm_id, name: geographyLabel(selected), center: selected.center, geojson: geometry }]
    : [];
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="flex w-full flex-col gap-0 p-0 sm:max-w-3xl md:max-w-5xl">
        <SheetHeader className="border-b px-5 py-4 text-left sm:px-6">
          ...
          <SheetDescription>Choose the jurisdiction associated with {entityName || "this entity"}.</SheetDescription>
        </SheetHeader>
          ...
          <div className="flex min-h-0 flex-col border-r">
            <div className="space-y-3 border-b p-4">
              <div className="relative">
                ...
                <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search India, Mumbai, BMC..." className="pl-9" />
              </div>
              {path.length > 0 && !search && (
                <div className="flex items-center gap-1 overflow-hidden text-xs text-muted-foreground">
                  <button type="button" onClick={() => { setParent(null); setPath([]); setSearch(""); }} className="shrink-0 hover:text-foreground">India</button>
                  {path.slice(1).map((item) => (
                    <span key={item.id} className="flex min-w-0 items-center gap-1"><ChevronRight className="h-3 w-3 shrink-0" /><span className="truncate">{geographyLabel(item)}</span></span>
                  ))}
                </div>
              )}
            </div>
            ...
            <ScrollArea className="min-h-0 flex-1">
              <RadioGroup value={selectedId} onValueChange={(value) => { const item = items.find((candidate) => candidate.id === value); if (item) selectGeography(item); }} className="p-2">
                {path.length > 0 && !search && (
                  <button type="button" onClick={goBack} className="mb-1 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-muted-foreground hover:bg-muted hover:text-foreground"><ArrowLeft className="h-4 w-4" />Back</button>
                )}
                {isLoading ? (
                  <div className="flex items-center gap-2 px-3 py-8 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Loading boundaries...</div>
                ) : items.length === 0 ? (
                  <div className="px-3 py-8 text-center text-sm text-muted-foreground">No matching boundaries found.</div>
                ) : (
                  items.map((item) => {
                    const isSelected = selected?.id === item.id;
                    return (
                      <div key={item.id} className={`flex items-center gap-2 rounded-lg px-2 py-1 ${isSelected ? "bg-accent" : ""}`}>
                        <label htmlFor={`geography-${item.id}`} className="flex min-w-0 flex-1 cursor-pointer items-center gap-3 rounded-md px-2 py-2 text-left hover:bg-muted">
                          <RadioGroupItem id={`geography-${item.id}`} value={item.id} className="shrink-0" />
                        ...
                        {!search && <Button type="button" size="icon" variant="ghost" className="h-8 w-8 shrink-0" onClick={() => openChildren(item)} title={`Browse inside ${geographyLabel(item)}`}><ChevronRight className="h-4 w-4" /></Button>}
                      </div>
                    );
                  })
                )}
        ...
        <SheetFooter className="border-t bg-background px-5 py-4 sm:px-6">
          {selected ? (
            <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0"><p className="truncate text-sm font-semibold">{geographyLabel(selected)}</p><p className="text-xs text-muted-foreground">{geographyTypeLabel(selected)}</p></div>
              <div className="flex shrink-0 gap-2"><Button type="button" variant="outline" onClick={() => onOpenChange?.(false)} disabled={isSetting}>Cancel</Button><Button type="button" onClick={handleSave} disabled={isSetting || selected.id === currentGeographyId || loadingGeometry}>{isSetting ? "Saving..." : submitLabel}</Button></div>
            </div>
          ) : (
            <div className="w-full text-xs text-muted-foreground">India is the default starting point. Browse down through the administrative hierarchy or search for a boundary directly.</div>
          )}
        </SheetFooter>
      </SheetContent>
    </Sheet>
