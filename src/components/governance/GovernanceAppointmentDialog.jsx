...
export default function GovernanceAppointmentDialog({
  ...
  useEffect(() => {
    ...
    if (selected && selected.appointing_organization_id !== selectedOrganizationId) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const result = await supabase.rpc("upsert_organization", {
        ...
        p_person_name: positionMode ? (creatingPerson ? newPersonName.trim() : selectedPerson?.name || "") : "",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          ...
          <div className="space-y-4 py-2">
            {positionMode ? (
              <div className="rounded-lg border bg-muted/30 p-3">
                ...
                <div className="mt-1 text-sm font-medium">{selectedPosition?.name || "Position"}</div>
                <div className="text-xs text-muted-foreground">{selectedOrganization?.name || "Organization"}</div>
              </div>
            ) : (
              <div className="rounded-lg border bg-muted/30 p-3">
                <div className="text-xs text-muted-foreground">Person</div>
                <div className="mt-1 text-sm font-medium">{selectedPerson?.name || "Person"}</div>
              </div>
            )}
            ...
            {personMode && (
              <div className="space-y-2">
                ...
                <SearchableSelect
                  ...
                  placeholder={selectedOrganizationId ? "Choose a position" : "Choose an organization first"}
            ...
            {creatingPerson && (
              <div className="space-y-4 rounded-lg border p-4">
                ...
                <ImageUpload
                  bucket="governance"
                  path="governance/person/draft/image"
                ...
                />
                <div className="space-y-2">
                  <Label htmlFor="governance-appointment-new-person">Name</Label>
                  <Input id="governance-appointment-new-person" value={newPersonName} onChange={(event) => setNewPersonName(event.target.value)} placeholder="e.g. Jane Doe" disabled={saving} autoFocus />
                </div>
              </div>
            )}

            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2"><Label>Start date</Label><Input type="date" value={startedAt} onChange={(event) => setStartedAt(event.target.value)} disabled={saving} /></div>
              <div className="space-y-2"><Label>End date</Label><Input type="date" value={endedAt} onChange={(event) => setEndedAt(event.target.value)} disabled={saving} /></div>
            </div>

            <div className="flex items-center justify-between rounded-lg border p-3">
              <div><div className="text-sm font-medium">Primary appointment</div><div className="text-xs text-muted-foreground">Use this for the person’s current or principal appointment.</div></div>
              <Checkbox checked={isPrimary} disabled={saving} onCheckedChange={(checked) => setIsPrimary(!!checked)} />
            </div>
          </div>
        )}
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={save} disabled={saving || loading}>{saving ? "Saving..." : positionMode ? "Add person" : "Add position"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
