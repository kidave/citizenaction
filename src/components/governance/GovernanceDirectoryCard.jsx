    ...
    <Card className="h-full transition-colors hover:border-primary/40 hover:bg-accent/30">
      <CardContent className="flex min-h-[92px] items-start gap-3 p-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {tab === "positions" && entity.parent_name && <div className="mt-2 min-w-0 truncate text-xs text-muted-foreground">{entity.parent_name}</div>}
          {tab === "positions" && holder && <div className="mt-1 flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground"><UserRound className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{holder}</span></div>}
          ...
          {tab === "people" && entity.current_holder_name && <div className="mt-2 min-w-0 truncate text-xs text-muted-foreground">{entity.current_holder_name}</div>}
          {tab === "people" && entity.geography_name && <div className="mt-1 flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground"><MapPin className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{entity.geography_name}</span></div>}
          {tab === "organizations" && (entity.geography_name || entity.category_name) && <div className="mt-2 flex min-w-0 items-center gap-2 text-[11px] text-muted-foreground">{entity.geography_name && <span className="truncate">{entity.geography_name}</span>}{entity.geography_name && entity.category_name && <span>·</span>}{entity.category_name && <span className="truncate">{entity.category_name}</span>}</div>}
        </div>
      </CardContent>
    </Card>
  );
