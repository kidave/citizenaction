...
export default function GovernanceEntityModal({ open, onOpenChange, entity, parent, childEntities = [], canEdit = false, onSelect, onSaved, onDeleted, onAddRelation, onEditRelations, onAddGeography: externalAddGeography, onChangeGeography: externalChangeGeography, onRemoveGeography: externalRemoveGeography, categories = [] }) {
  ...
  const { editing, draft, updateDraft, closeEditing, startEditing, validate } = useGovernanceEntityForm(entity);
  ...
  async function handleSave() {
    ...
    try { const saved = await updateEntity({ entity, draft }); closeEditing(); onSaved?.(saved); toast.success("Governance entity updated"); }
  ...
  async function handleDelete() {
    ...
    try { await deleteEntity(entity.id); toast.success("Governance entity deleted"); onDeleted?.(entity); onOpenChange?.(false); }
  ...
  const removeGeographyLink = externalRemoveGeography || (async () => { try { await removeGeography({ governanceId: entity.id }); await onSaved?.(entity); toast.success("Geography removed"); } catch (error) { toast.error(error?.message || "Unable to remove geography"); } });

  return <>
    <Dialog open={open} onOpenChange={(value) => { if (!value && !saving) closeEditing(); if (!saving) onOpenChange?.(value); }}>
