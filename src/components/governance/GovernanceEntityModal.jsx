...
const ENTITY_TYPES = ["authority", "unit", "position", "person", "organisation", "committee", "programme", "project", "ministry", "department", "division", "office", "ward", "station"];
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
        ...
        {editing ? (
          ...
          <div className="space-y-5">
            ...
            {(parent || canEdit) && (
              <section>
                <div className="flex items-center justify-between gap-3"><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Parent</p>{canEdit && <Button type="button" variant="ghost" size="sm" onClick={() => onChangeParent?.(entity)}>Change</Button>}</div>
                {parent ? <button type="button" onClick={() => onSelect?.(parent)} className="mt-2 w-full rounded-lg border p-3 text-left hover:bg-accent"><span className="block truncate text-sm font-medium">{getGovernanceLabel(parent)}</span><span className="text-xs text-muted-foreground">{formatType(parent)}</span></button> : <p className="mt-2 text-sm text-muted-foreground">No parent recorded.</p>}
              </section>
            )}
            ...
            <section>
              ...
              {childEntities.length ? <div className="mt-2 max-h-64 space-y-1.5 overflow-y-auto">{childEntities.map((child) => <button type="button" key={child.id} onClick={() => onSelect?.(child)} className="flex w-full items-center justify-between rounded-md px-3 py-2 text-left hover:bg-muted"><span className="min-w-0 truncate text-sm font-medium">{getGovernanceLabel(child)}</span><span className="ml-3 shrink-0 text-xs text-muted-foreground">{formatType(child)}</span></button>)}</div> : <p className="mt-2 text-sm text-muted-foreground">No child entities recorded yet.</p>}
            </section>

            <div className="space-y-1 text-xs text-muted-foreground">{entity.valid_from && <p>Valid from {new Date(entity.valid_from).toLocaleDateString()}</p>}{entity.valid_to && <p>Valid to {new Date(entity.valid_to).toLocaleDateString()}</p>}</div>
