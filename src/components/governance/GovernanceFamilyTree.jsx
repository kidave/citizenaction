...
import { getGovernanceLabel, getGovernanceTreeLabel, formatGovernanceType, getGovernanceInitials } from "@/utils/governance";
...
function TreeNode({ node, expandedIds, onToggle, selectedId, onSelect, canEdit, onAddParent, onAddChild, onChangeParent }) {
  ...
  return (
    <li className="flex min-w-0 flex-col items-center">
      <div className="flex w-full min-w-0 items-center justify-center gap-1">
        {textOnly ? (
          ...
          <button type="button" onClick={() => onSelect?.(node)} className={cn("group flex w-[210px] min-w-0 items-center gap-3 rounded-xl border bg-card px-4 py-3 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md", selected && "border-primary ring-2 ring-primary/15")} title={getGovernanceLabel(node)}>
            <Avatar className="h-10 w-10 shrink-0 rounded-lg"><AvatarImage src={node.image_url || undefined} alt="" /><AvatarFallback className="rounded-lg bg-muted">{node.entity_type === "authority" ? <Landmark className="h-4 w-4" /> : getGovernanceInitials(label)}</AvatarFallback></Avatar>
            <span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{label}</span><span className="mt-0.5 block truncate text-xs text-muted-foreground">{formatGovernanceType(node)}</span></span>
          </button>
        )}
        {actions && <MenuButton onAddParent={() => onAddParent?.(node)} onAddChild={() => onAddChild?.(node)} onChangeParent={() => onChangeParent?.(node)} />}
      ...
      {hasChildren && expanded && <div className="mt-8 w-full overflow-visible px-1"><ul className="grid w-full items-start gap-4" style={{ gridTemplateColumns: `repeat(${node.children.length}, minmax(0, 1fr))` }}>{node.children.map((child) => <TreeNode key={child.id} node={child} expandedIds={expandedIds} onToggle={onToggle} selectedId={selectedId} onSelect={onSelect} canEdit={canEdit} onAddParent={onAddParent} onAddChild={onAddChild} onChangeParent={onChangeParent} />)}</ul></div>}
    </li>
  );
}

export default function GovernanceFamilyTree({ records = [], selectedId = null, onSelect, className, initialExpandedIds = [], canEdit = false, onAddParent, onAddChild, onChangeParent }) {
  ...
  return <div className={cn("h-full w-full overflow-auto rounded-2xl border bg-background/50", className)}><div className="flex min-h-full min-w-[720px] items-start justify-center gap-16 p-8 sm:p-12">{roots.map((root) => <TreeNode key={root.id} node={root} expandedIds={expandedIds} onToggle={toggle} selectedId={selectedId} onSelect={onSelect} canEdit={canEdit} onAddParent={onAddParent} onAddChild={onAddChild} onChangeParent={onChangeParent} />)}</div></div>;
}
