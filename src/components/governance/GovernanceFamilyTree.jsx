...
const TEXT_TYPES = new Set(["ministry", "department"]);
const MIN_ZOOM = 0.55;
const MAX_ZOOM = 1.6;
const ZOOM_STEP = 0.1;
...
function TreeNode({ node, expandedIds, onToggle, selectedId, onSelect, canEdit, actions, onOpenOrganization, onEdit, onAddRelation, onEditRelations, onAddGeography, onChangeGeography, onRemoveGeography, onDelete }) {
  ...
  const textOnly = TEXT_TYPES.has(node.type);
  ...
  const content = textOnly ? <button type="button" onClick={openCard} className={cn("w-full min-w-0 rounded-md px-3 py-2 text-center transition-colors hover:bg-muted", selected && "bg-accent ring-1 ring-primary/25")} title={getGovernanceLabel(node)}><span className="block truncate text-sm font-medium">{label}</span><span className="mt-0.5 block truncate text-xs text-muted-foreground">{formatGovernanceType(node)}</span></button> : <button type="button" onClick={openCard} className={cn("group flex w-full min-w-0 items-center gap-3 rounded-xl border bg-card px-4 py-3 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md", selected && "border-primary ring-2 ring-primary/15")} title={getGovernanceLabel(node)}><Avatar className="h-10 w-10 shrink-0 rounded-lg"><AvatarImage src={node.image_url || undefined} alt="" /><AvatarFallback className="rounded-lg bg-muted">{node.type === "authority" ? <Landmark className="h-4 w-4" /> : getGovernanceInitials(label)}</AvatarFallback></Avatar><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{label}</span><span className="mt-0.5 block truncate text-xs text-muted-foreground">{formatGovernanceType(node)}</span></span></button>;
