...
function TreeNode({ node, expandedIds, onToggle, selectedId, onSelect, canEdit, actions, onOpenOrganization, onEdit, onAddRelation, onEditRelations, onAddGeography, onChangeGeography, onRemoveGeography, onDelete }) {
  ...
  const content = textOnly ? (
    <button type="button" onClick={openCard} className={cn("w-full min-w-0 rounded-md px-3 py-2 text-center transition-colors hover:bg-muted", selected && "bg-accent ring-1 ring-primary/25")} title={getGovernanceLabel(node)}>
      <span className="block truncate text-sm font-medium">{label}</span>
      <span className="mt-0.5 block truncate text-xs text-muted-foreground">{formatGovernanceType(node)}</span>
    </button>
  ) : (
    <button type="button" onClick={openCard} className={cn("group flex w-full min-w-0 items-center gap-3 rounded-xl border bg-card px-4 py-3 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md", selected && "border-primary ring-2 ring-primary/15")} title={getGovernanceLabel(node)}>
      ...
      <div className="flex w-full min-w-0 items-center justify-center gap-1">
        <ContextMenu>
          ...
          <ContextMenuContent onCloseAutoFocus={(event) => event.preventDefault()}>
            {editable && <ContextMenuItem onSelect={menuAction(onEdit)}><Pencil className="mr-2 h-4 w-4" />Edit</ContextMenuItem>}
            <ContextMenuItem onSelect={menuAction(onOpenOrganization)}><ExternalLink className="mr-2 h-4 w-4" />Open organization</ContextMenuItem>
            {editable && (
              <>
                <ContextMenuSeparator />
                <ContextMenuItem onSelect={menuAction(onAddRelation)}><Plus className="mr-2 h-4 w-4" />Add relation</ContextMenuItem>
                {actions?.hasRelations?.(node) && <ContextMenuItem onSelect={menuAction(onEditRelations)}><Pencil className="mr-2 h-4 w-4" />Edit relations</ContextMenuItem>}
                {node.geography_id ? <><ContextMenuItem onSelect={menuAction(onChangeGeography)}>Change geography</ContextMenuItem><ContextMenuItem onSelect={menuAction(onRemoveGeography)}>Remove geography</ContextMenuItem></> : <ContextMenuItem onSelect={menuAction(onAddGeography)}>Add geography</ContextMenuItem>}
              </>
            )}
            {editable && onDelete && <><ContextMenuSeparator /><ContextMenuItem className="text-destructive focus:text-destructive" onSelect={menuAction(onDelete)}><Trash2 className="mr-2 h-4 w-4" />Delete</ContextMenuItem></>}
          </ContextMenuContent>
        </ContextMenu>
...
export default function GovernanceFamilyTree({ records = [], selectedId = null, onSelect, className, initialExpandedIds = [], canEdit = false, onOpenOrganization, onEdit, onAddRelation, onEditRelations, onAddGeography, onChangeGeography, onRemoveGeography, onDelete }) {
  ...
  return (
    <div className={cn("relative h-full min-h-[calc(100vh-7rem)] w-full overflow-hidden bg-background", dragging ? "cursor-grabbing" : "cursor-grab", className)} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={endDrag} onPointerCancel={endDrag} onWheel={handleWheel}>
      <div className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-start justify-end p-3 sm:p-4">
        <div className="pointer-events-auto flex items-center rounded-lg border bg-background/95 p-1 shadow-sm backdrop-blur">
          ...
          <button type="button" className="min-w-[3.5rem] px-2 text-xs font-medium tabular-nums text-muted-foreground hover:text-foreground" onClick={() => setZoom(DEFAULT_ZOOM)} aria-label="Reset zoom" title="Reset zoom">{Math.round(zoom * 100)}%</button>
          ...
          <Button type="button" variant="ghost" size="icon" className="h-8 w-8" onClick={resetView} disabled={zoom === DEFAULT_ZOOM && pan.x === 0 && pan.y === 0} aria-label="Reset view"><RotateCcw className="h-4 w-4" /></Button>
        </div>
      </div>
      ...
      <div className="absolute left-1/2 top-1/2 w-max origin-center select-none transition-transform duration-100 ease-out" style={{ transform: `translate(calc(-50% + ${pan.x}px), calc(-50% + ${pan.y}px)) scale(${zoom})` }}>
        <ul className="flex items-start justify-center gap-10 p-16 sm:p-24">
          {roots.map((root) => <TreeNode key={root.id} node={root} expandedIds={expandedIds} onToggle={(id) => setExpandedIds((current) => { const next = new Set(current); if (next.has(id)) next.delete(id); else next.add(id); return next; })} selectedId={selectedId} onSelect={onSelect} canEdit={canEdit} actions={{ hasRelations: (item) => Boolean(item.parent_id || item.children?.length) }} onOpenOrganization={onOpenOrganization} onEdit={onEdit} onAddRelation={onAddRelation} onEditRelations={onEditRelations} onAddGeography={onAddGeography} onChangeGeography={onChangeGeography} onRemoveGeography={onRemoveGeography} onDelete={onDelete} />)}
        </ul>
      </div>
    </div>
  );
