...
export default function GovernanceRelationDialog({ open, onOpenChange, mode = "add-relation", sourceEntity, candidates = [], categories = [], onCompleted }) {
  const [step, setStep] = useState("choose-relation");
  ...
  useEffect(() => {
    if (!open) {
      setStep(mode === "add-relation" ? "choose-relation" : mode === "change-parent" ? "parent" : "choose");
  ...
  const effectiveMode = mode === "add-relation"
  ...
  const resetToRelationChoice = () => {
    setRelationType(null);
    setStep("choose-relation");
    setExistingId("");
    setName("");
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (effectiveMode === "change-parent") {
        ...
        const rpc = effectiveMode === "add-parent" ? "add_governance_parent" : "add_governance_child";
        const payload = {
          p_name: name.trim(),
        ...
        payload[effectiveMode === "add-parent" ? "p_child_id" : "p_parent_id"] = sourceEntity.id;
          ...
          {step === "choose-relation" && effectiveMode !== "change-parent" ? (
            ...
            <>
              ...
              {step === "choose" ? (
                ...
                <Field label={effectiveMode === "add-child" ? "Child entity" : "Parent entity"}>
                  <Select value={existingId} onValueChange={setExistingId}>
