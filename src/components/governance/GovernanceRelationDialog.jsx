...
export default function GovernanceRelationDialog({ open, onOpenChange, mode = "add-child", sourceEntity, candidates = [], categories = [], onCompleted }) {
  ...
  useEffect(() => {
    if (!open) {
      setStep("choose"); setExistingId(""); setName(""); setEntityType("unit"); setUnitType("unit");
  ...
  const save = async () => {
    ...
    try {
      ...
      if (mode === "change-parent") {
        const result = await supabase.rpc("set_governance_parent", { p_child_id: sourceEntity.id, p_parent_id: existingId === "none" ? null : existingId || null });
        if (!result || result.error) throw result?.error || new Error("Unable to change parent");
        ...
        const result = await supabase.rpc("set_governance_parent", { p_child_id: mode === "add-child" ? existingId : sourceEntity.id, p_parent_id: mode === "add-child" ? sourceEntity.id : existingId });
        ...
        const rpc = mode === "add-parent" ? "add_governance_parent" : "add_governance_child";
        ...
        payload[mode === "add-parent" ? "p_child_id" : "p_parent_id"] = sourceEntity.id;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        ...
        <div className="space-y-4 py-2">
          ...
          {mode === "change-parent" ? (
            <Field label="Parent"><Select value={existingId} onValueChange={setExistingId}><SelectTrigger><SelectValue placeholder="Choose a parent" /></SelectTrigger><SelectContent className="max-h-72"><SelectItem value="none">No parent — make independent</SelectItem>{filtered.map((item) => <SelectItem key={item.id} value={item.id}>{getGovernanceLabel(item)}</SelectItem>)}</SelectContent></Select></Field>
          ) : step === "choose" ? (
            <div className="grid gap-3 sm:grid-cols-2"><Button type="button" variant="outline" className="h-auto justify-start p-4 text-left" onClick={() => setStep("existing")}><div><div className="font-medium">Use an existing entity</div><div className="mt-1 text-xs text-muted-foreground">Connect a record already in Governance.</div></div></Button><Button type="button" variant="outline" className="h-auto justify-start p-4 text-left" onClick={() => setStep("create")}><div><div className="font-medium">Create a new entity</div><div className="mt-1 text-xs text-muted-foreground">Add a new record here.</div></div></Button></div>
          ) : step === "existing" ? (
            <Field label={mode === "add-child" ? "Child entity" : "Parent entity"}><Select value={existingId} onValueChange={setExistingId}><SelectTrigger><SelectValue placeholder="Choose an entity" /></SelectTrigger><SelectContent className="max-h-72">{filtered.map((item) => <SelectItem key={item.id} value={item.id}>{getGovernanceLabel(item)}</SelectItem>)}</SelectContent></Select></Field>
          ) : (
            <div className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Entity type"><Select value={entityType} onValueChange={setEntityType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{GOVERNANCE_ENTITY_TYPES.map((type) => <SelectItem key={type} value={type}>{type.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase())}</SelectItem>)}</SelectContent></Select></Field>
                <Field label="Structural role"><Select value={unitType} onValueChange={setUnitType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{GOVERNANCE_UNIT_TYPES.map((type) => <SelectItem key={type} value={type}>{type.replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase())}</SelectItem>)}</SelectContent></Select></Field>
              </div>
