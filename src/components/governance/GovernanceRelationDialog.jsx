...
import { getGovernanceLabel, getDefaultGovernanceUnitType, getGovernanceTypeLabel, governanceRequiresValidTo, wouldCreateGovernanceCycle, GOVERNANCE_ENTITY_TYPES, GOVERNANCE_UNIT_TYPES, GOVERNANCE_STATUS_OPTIONS } from "@/utils/governance";
  ...
  const save = async () => {
    ...
    try {
      ...
      if (mode === "change-parent") {
        if (existingId !== "none" && !existingId) return toast.error("Choose a parent or select No parent");
        if (existingId !== "none" && wouldCreateGovernanceCycle(sourceEntity.id, existingId, candidates)) return toast.error("That relationship would create a cycle");
        const { error } = await supabase.from("governance_unit").update({ parent_entity_id: existingId === "none" ? null : existingId }).eq("entity_id", sourceEntity.id);
      ...
      if (step === "existing") {
        ...
        if (mode === "add-child" && wouldCreateGovernanceCycle(sourceEntity.id, existingId, candidates)) return toast.error("That relationship would create a cycle");
        if (mode === "add-parent" && wouldCreateGovernanceCycle(existingId, sourceEntity.id, candidates)) return toast.error("That relationship would create a cycle");
      ...
      if (governanceRequiresValidTo(status) && !validTo) return toast.error("Add Valid to when the entity becomes inactive");
      ...
      const { data: governance, error: governanceError } = await supabase.from("governance").insert({
        ...
        valid_to: governanceRequiresValidTo(status) && validTo ? `${validTo}T23:59:59.999Z` : null,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        ...
        <div className="space-y-4 py-2">
          <div className="rounded-lg border bg-muted/30 p-3 text-sm"><span className="font-medium">{getGovernanceLabel(sourceEntity)}</span><span className="mx-1 text-muted-foreground">·</span><span className="text-muted-foreground">{mode === "add-child" ? "parent of" : mode === "add-parent" ? "child of" : "new parent for"}</span></div>

          {mode === "change-parent" ? (
            <Field label="Parent">
              <Select value={existingId} onValueChange={setExistingId}>
                <SelectTrigger><SelectValue placeholder="Choose a parent" /></SelectTrigger>
                <SelectContent className="max-h-72"><SelectItem value="none">No parent — make independent</SelectItem>{filtered.map((item) => <SelectItem key={item.id} value={item.id}>{getGovernanceLabel(item)}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
          ) : step === "choose" ? (
            ...
            <Field label={mode === "add-child" ? "Child entity" : "Parent entity"}>
              <Select value={existingId} onValueChange={setExistingId}>
                <SelectTrigger><SelectValue placeholder="Choose an entity" /></SelectTrigger>
                <SelectContent className="max-h-72">{filtered.map((item) => <SelectItem key={item.id} value={item.id}>{getGovernanceLabel(item)} · {getGovernanceTypeLabel(item)}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
          ) : (
            <div className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Entity type"><Select value={entityType} onValueChange={setEntityType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{GOVERNANCE_ENTITY_TYPES.map((type) => <SelectItem key={type} value={type}>{getGovernanceTypeLabel({ entity_type: type })}</SelectItem>)}</SelectContent></Select></Field>
                <Field label="Structural role"><Select value={unitType} onValueChange={setUnitType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{GOVERNANCE_UNIT_TYPES.map((type) => <SelectItem key={type} value={type}>{getGovernanceTypeLabel({ entity_type: type })}</SelectItem>)}</SelectContent></Select></Field>
              </div>
              ...
              <ImageUpload bucket="governance" path={`governance/new-${sourceEntity?.id || "entity"}/logo`} value={imageUrl} onChange={(value) => setImageUrl(value || null)} label="Logo" helperText="PNG, JPG or WebP · up to 5 MB" disabled={saving} />
            </div>
          )}
        </div>
        <DialogFooter>
