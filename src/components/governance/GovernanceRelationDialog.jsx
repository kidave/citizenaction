  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader><DialogTitle className="flex items-center gap-2"><GitBranch className="h-4 w-4" />{isEdit ? "Edit relations" : "Add relation"}</DialogTitle></DialogHeader>
        ...
        <DialogFooter>{((!isEdit && step !== "relation") || (isEdit && step !== "edit")) && <Button type="button" variant="ghost" onClick={isEdit ? () => setStep("edit") : back} disabled={saving}><ArrowLeft className="mr-2 h-4 w-4" />Back</Button>}{isEdit && step === "edit-parent" && <Button type="button" onClick={save} disabled={saving || !existingId}>{saving ? "Saving..." : "Save parent"}</Button>}{!isEdit && (step === "existing" || step === "create") && <Button type="button" onClick={save} disabled={saving || (step === "existing" && !existingId)}>{saving ? "Saving..." : step === "create" ? "Create relation" : "Save relation"}</Button>}</DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
