...
import {
  getGovernanceLabel,
  getDefaultGovernanceUnitType,
  GOVERNANCE_ENTITY_TYPES,
  GOVERNANCE_UNIT_TYPES,
...
export default function GovernanceRelationDialog({
  ...
  const filtered = useMemo(
    () => candidates.filter((item) => item.id !== sourceEntity?.id),
    [candidates, sourceEntity],
  );
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="flex w-full flex-col gap-0 p-0 sm:max-w-xl md:max-w-2xl">
            ...
            {isEdit ? (
              <>
                <div className="rounded-lg border bg-muted/30 p-3 text-sm">
                  ...
                  <span className="text-muted-foreground">manage reporting relationships</span>
                </div>
                  ...
                  <div className="flex items-center gap-2 rounded-lg border p-3">
                    <span className="min-w-0 flex-1 truncate text-sm">
                      {sourceEntity?.parent_id
                        ? getGovernanceLabel(filtered.find((item) => item.id === sourceEntity.parent_id)) || "Current parent"
                        : "No parent"}
                    </span>
                    <Button
                      type="button"
                  ...
                  <Field label="Parent">
                    <Select value={existingId} onValueChange={setExistingId}>
                      ...
                      <SelectContent className="max-h-72">
                        ...
                        {filtered.map((item) => (
                          <SelectItem key={item.id} value={item.id}>{getGovernanceLabel(item)}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  </Field>
              ...
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">How should the new relation connect to this entity?</p>
                <div className="grid gap-3 sm:grid-cols-2">
                  <Button type="button" variant="outline" className="h-auto justify-start p-4 text-left" onClick={() => chooseRelation("parent-of")}>
                    <div><div className="font-medium">Parent of</div><div className="mt-1 text-xs text-muted-foreground">Connect an entity that reports to this one.</div></div>
                  </Button>
                  <Button type="button" variant="outline" className="h-auto justify-start p-4 text-left" onClick={() => chooseRelation("child-of")}>
                    <div><div className="font-medium">Child of</div><div className="mt-1 text-xs text-muted-foreground">Connect this entity to the entity it reports to.</div></div>
                  </Button>
                </div>
              </div>
            ) : step === "target" ? (
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">Choose how you want to add the connected entity.</p>
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <Button type="button" variant="outline" className="h-auto justify-start p-4 text-left" onClick={() => setStep("create")}>
                    <div><div className="font-medium">Create a new entity</div><div className="mt-1 text-xs text-muted-foreground">Add a new record here.</div></div>
                  </Button>
                </div>
              </div>
            ) : step === "existing" ? (
              <Field label={relationType === "parent-of" ? "Child entity" : "Parent entity"}>
                <Select value={existingId} onValueChange={setExistingId}>
        ...
        <SheetFooter className="border-t bg-background px-5 py-4 sm:px-6">
          {((!isEdit && step !== "relation") || (isEdit && step !== "edit")) && (
            <Button type="button" variant="ghost" onClick={isEdit ? () => setStep("edit") : back} disabled={saving}>
          ...
          {!isEdit && (step === "existing" || step === "create") && (
            <Button type="button" onClick={save} disabled={saving || (step === "existing" && !existingId)}>
              {saving ? "Saving..." : step === "create" ? "Create relation" : "Save relation"}
            </Button>
          )}
        </SheetFooter>
      </SheetContent>
