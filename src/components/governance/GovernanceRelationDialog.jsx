...
function defaultUnitType(entityType) {
  const map = { authority: "authority", ministry: "ministry", department: "department", division: "division", office: "office", ward: "ward", station: "station" };
  return map[entityType] || "unit";
}
  ...
  const save = async () => {
    ...
    try {
      ...
      if (mode === "change-parent") {
        ...
        if (existingId !== "none" && wouldCreateCycle(sourceEntity.id, existingId, candidates)) return toast.error("That relationship would create a cycle");
      ...
      if (step === "existing") {
        ...
        if (mode === "add-child" && wouldCreateCycle(sourceEntity.id, existingId, candidates)) return toast.error("That relationship would create a cycle");
          ...
          {mode === "change-parent" ? (
            ...
            <Field label={mode === "add-child" ? "Child entity" : "Parent entity"}>
              <Select value={existingId} onValueChange={setExistingId}>
                ...
                <SelectContent className="max-h-72">{filtered.map((item) => <SelectItem key={item.id} value={item.id}>{getGovernanceLabel(item)} · {formatType(item.entity_type)}</SelectItem>)}</SelectContent>
              </Select>
            </Field>
          ) : (
