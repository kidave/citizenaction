...
const TYPE_LABELS = { 4: "State / Union territory", 5: "District", 6: "Subdistrict / Taluka", 7: "Intermediate area", 8: "Local government", 9: "Zone", 10: "Ward" };
...
function SelectField({ label, optional = true, value, placeholder, disabled, loading, items, onChange, emptyLabel = "No mapped boundaries" }) {
  return (
        ...
        <SelectContent className="max-h-72">
          {items.map((item) => (
            <SelectItem key={`${item.osm_type}:${item.osm_id}`} value={String(item.osm_id)}>
              {item.name}
              {item.local_authority && <span className="ml-2 text-xs text-muted-foreground">{displayType(item)}</span>}
            </SelectItem>
          ))}
          {!loading && !items.length && <div className="px-2 py-1.5 text-sm text-muted-foreground">{emptyLabel}</div>}
        </SelectContent>
      </Select>
    </div>
  );
...
export default function OSMJurisdictionPicker({ value, onChange, disabled = false }) {
  ...
  const loadStateChildren = useCallback(async (nextState) => {
    ...
    try {
      ...
      if (districtResult.status === "rejected") setError("District boundaries could not be loaded, but local-government boundaries are available.");
      else if (localGovernmentResult.status === "rejected") setError("Local-government boundaries could not be loaded, but district boundaries are available.");
    } catch {
      setDistricts([]);
      setLocalBodies([]);
      resetAdminChildren();
  ...
  const loadLocalBodyChildren = useCallback(async (nextLocalBody) => {
    ...
    try {
      ...
      const [nextZones, nextWards] = await Promise.all([
        fetchBoundaryList({ parentOsmId: nextLocalBody.osm_id, localAuthority: "municipal_corporation_zone", adminLevel: 9 }),
        fetchBoundaryList({ parentOsmId: nextLocalBody.osm_id, localAuthority: "ward", adminLevel: 10 }),
      ]);
      setZones(nextZones);
      setWards(nextWards);
      setZoneId("");
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border bg-muted/20 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-muted-foreground">India is the default country. Administrative boundaries and local-government boundaries are separate hierarchies.</p>
          </div>
        </div>
      </div>
      ...
      {loadingStates ? (
        ...
        <div className="space-y-3">
          <div className="grid gap-3 sm:grid-cols-2">
            <SelectField label="State / Union territory" optional={false} value={stateId} placeholder="Choose a state or Union territory" disabled={disabled} loading={loadingStates} items={states} onChange={handleStateChange} />
            <SelectField label="District" value={districtId} placeholder={!stateId ? "Choose a state first" : loadingChildren ? "Loading…" : "Choose a district"} disabled={disabled || !stateId || loadingChildren} loading={loadingChildren} items={districts} onChange={handleDistrictChange} />
          </div>
          ...
          {stateId && (
            <div className="rounded-xl border p-4 space-y-3">
              ...
              <SelectField label="Subdistrict / Taluka" value={subdistrictId} placeholder={!districtId ? "Choose a district first" : "Choose a subdistrict"} disabled={disabled || !districtId || loadingChildren} loading={loadingChildren} items={subdistricts} onChange={handleSubdistrictChange} />
            </div>
          )}
          ...
          {stateId && (
            <div className="rounded-xl border p-4 space-y-3">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">Independent of district and subdistrict. Choose the governing civic body, then its zone and ward.</p>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <SelectField label="Region / Local government" value={localBodyId} placeholder="Choose a municipal corporation, municipality or city council" disabled={disabled || loadingChildren} loading={loadingChildren} items={localBodies} onChange={handleLocalBodyChange} />
                <SelectField label="Zone" value={zoneId} placeholder={!localBodyId ? "Choose a local government first" : "Choose a zone"} disabled={disabled || !localBodyId || loadingChildren} loading={loadingChildren} items={zones} onChange={handleZoneChange} />
              </div>
              <SelectField label="Ward" value={wardId} placeholder={!localBodyId ? "Choose a local government first" : "Choose a ward"} disabled={disabled || !localBodyId || loadingChildren} loading={loadingChildren} items={wards} onChange={handleWardChange} />
            </div>
          )}
        </div>
      )}
      ...
      <div className="overflow-hidden rounded-xl border">
        <div className="border-b bg-muted/20 px-3 py-2 text-xs text-muted-foreground">{currentSelection ? "Boundary preview" : "Select a boundary above to preview it on the map."}</div>
        <div className="relative h-80">
          <LeafletMap lat={mapCenter.lat} lng={mapCenter.lng} boundary={currentSelection?.geojson || null} boundaries={[]} selectedBoundaryId={currentSelection?.osm_id || null} showMarker={false} zoom={currentSelection?.admin_level === 4 ? 5 : currentSelection?.admin_level === 5 ? 7 : 10} onChange={() => {}} />
        </div>
      </div>

      <div className="flex items-center justify-between gap-3 rounded-lg border px-3 py-2.5 text-xs text-muted-foreground">
        <span className="flex items-center gap-1.5"><Check className="h-3.5 w-3.5" />You can save any one selected level as the organisation&apos;s jurisdiction.</span>
