...
export default function PersonDirectory({ geographyId = null, selectionMode = null, selectedIds = [], selectedId = null, onSelect, organizationId: controlledOrganizationId = null }) {
  ...
  const query = useGovernanceDirectory({ tab: "people", search, organizationId: effectiveOrganizationId, geographyId });
  ...
  return (
    <div className="space-y-4">
      ...
      {!query.isLoading && !query.error && (
        ...
        ) : <div className="flex min-h-[30vh] items-center justify-center text-center"><div><p className="text-sm font-medium">No people found.</p><p className="mt-1 text-xs text-muted-foreground">Try another search or filter.</p></div></div>
      )}
    </div>
  );
}
