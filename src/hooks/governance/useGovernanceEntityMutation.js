...
export function useGovernanceEntityMutation() {
  ...
  const update = useMutation({
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: ["governance-entity-details", variables.entity.id] });
      return data;
    },
  });
  ...
  const remove = useMutation({
    mutationFn: async (entityId) => {
      const { error } = await supabase.rpc("delete_governance_entity", { p_entity_id: entityId });
      if (error) throw error;
    },
    onSuccess: (_, entityId) => {
      queryClient.removeQueries({ queryKey: ["governance-entity-details", entityId] });
    },
  });
