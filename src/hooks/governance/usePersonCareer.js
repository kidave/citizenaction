...
export function usePersonCareer(governanceId, enabled = true) {
  return useQuery({
    queryKey: queryKeys.users.personCareer(governanceId),
    ...
    queryFn: async () => {
      ...
      const careerResult = await supabase.rpc("get_person_career", { p_person_id: personResult.data.id });
      if (careerResult.error) throw careerResult.error;
      return { person: personResult.data, career: careerResult.data || [] };
    },
  });
}
