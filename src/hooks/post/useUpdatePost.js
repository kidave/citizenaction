  ...
  const mutation = useMutation({
    mutationFn: async ({ postId, postData }) => {
      ...
      try {
        ...
        const editorStoragePaths = new Set(editorAttachments.map((attachment) => attachment?.storage_path).filter(Boolean));
        ...
        const resolvedContentJson = resolveEditorImageUrls(postData.content_json, newUploadedAttachments);
        ...
        const { error: attachmentError } = await supabase.rpc("upsert_post_attachments", { p_post_id: postId, p_attachments: finalAttachments });
        if (attachmentError) throw attachmentError;

        const links = Array.isArray(postData.links) ? postData.links.map((link, index) => ({ url: link.url, type: link.type ?? "website", title: link.title ?? null, description: link.description ?? null, hostname: link.hostname ?? null, image_url: link.image_url ?? null, icon_url: link.icon_url ?? null, sort_order: index })) : [];
        const { error: linkError } = await supabase.rpc("upsert_post_links", { p_post_id: postId, p_links: links });
        if (linkError) throw linkError;

        const finalStoragePaths = new Set(finalAttachments.map((attachment) => attachment.storage_path).filter(Boolean));
        const removedAttachments = existingAttachments.filter((attachment) => attachment.storage_path && !finalStoragePaths.has(attachment.storage_path));
        if (removedAttachments.length) await deletePostAttachments(removedAttachments);
        return updatedPost;
      } catch (error) {
        if (newUploadedAttachments.length) {
          try { await deletePostAttachments(newUploadedAttachments); }
          catch (rollbackError) { if (process.env.NODE_ENV !== "production") console.error("Attachment rollback failed:", rollbackError); }
        }
        throw error;
      }
    },
    onSuccess: (updatedPost, variables) => {
      queryClient.setQueryData(queryKeys.posts.detail(variables.postId), (currentPost) => currentPost ? { ...currentPost, ...updatedPost } : updatedPost);
      queryClient.invalidateQueries({ queryKey: queryKeys.feed.all });
      toast.success("Post updated successfully", { id: "update-post" });
    },
    onError: (error) => {
      if (process.env.NODE_ENV !== "production") console.error("Update post error:", error);
      toast.error(error.message || "Failed to update post", { id: "update-post" });
    },
  });

  return { updatePost: mutation.mutateAsync, isUpdating: mutation.isPending };
}
