      ...
      try {
        const { data: existingAttachments, error: existingAttachmentsError } =
          await supabase
            .from("attachment")
            .select(
              "id,storage_path,thumbnail_path,thumbnail_url,public_url,file_name,mime_type,file_size,width,height,duration,sort_order,credit_name,credit_url",
            )
            .eq("post_id", postId)
        ...
        const retainedAttachments = existingAttachments
          ...
          .map((attachment) => {
            ...
            return {
              ...
              credit_name:
                editorAttachment?.credit_name ??
                attachment.credit_name ??
                null,
              credit_url:
                editorAttachment?.credit_url ??
        ...
        if (newFiles.length) {
          ...
          newUploadedAttachments = uploaded.map(
            (uploadedAttachment, index) => ({
              ...uploadedAttachment,
              credit_name: newFiles[index]?.credit_name ?? null,
              credit_url: newFiles[index]?.credit_url ?? null,
              description: newFiles[index]?.description ?? null,
            }),
          );
        }
        ...
        const { data: updatedPost, error: updateError } =
          await supabase.rpc("update_post", {
            p_post_id: postId,
            p_space_ids:
              postData.spaces?.map((space) => space.id) ?? [],
            ...
            p_governance_ids:
              postData.governance?.map((g) => g.id) ?? [],
          });
        if (updateError) throw updateError;

        const { error: attachmentError } = await supabase.rpc(
          "upsert_post_attachments",
          { p_post_id: postId, p_attachments: finalAttachments },
        );
