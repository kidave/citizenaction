    ...
    async function loadSpaceAdmin() {
      ...
      try {
        const { data: spaceData, error: spaceError } = await supabase
          .from("space")
          .select("id,name,slug,description,owner_user_id,logo_url,is_active")
          .eq("slug", slug)
          .eq("is_active", true)
          .single();
        ...
        setSpace({ ...spaceData, current_user_role: currentUserRole, is_owner: isOwner, is_admin: isAdmin });
        setPendingApplications(count || 0);
      } catch (err) {
        if (process.env.NODE_ENV !== "production") console.error("[SPACE ADMIN]", err);
