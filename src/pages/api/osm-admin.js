...
export default async function handler(req, res) {
  ...
  if (parentRelationId && stateName) {
    ...
    clauses.push(`rel["type"="boundary"]["boundary"="administrative"]${levelFilter}${nameFilter};`);
  } else if (city) {
    const escapedCity = escapeOverpassQuoted(city);
    clauses.push(`area["name"="${escapedCity}"]["boundary"="administrative"]->.cityArea;`);
    clauses.push(`rel["type"="boundary"]${boundaryFilter}${levelFilter}${localAuthorityFilter}${nameFilter}(area.cityArea);`);
  } else {
    clauses.push(`area["ISO3166-1"="IN"]["boundary"="administrative"]->.india;`);
    clauses.push(`rel["type"="boundary"]${boundaryFilter}${levelFilter}${localAuthorityFilter}${nameFilter}(area.india);`);
  }
  ...
  try {
    const response = await fetch("https://overpass-api.de/api/interpreter", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8", "User-Agent": "CitizenActionApp/1.0" },
