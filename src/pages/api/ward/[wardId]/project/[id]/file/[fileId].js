// pages/api/ward/[wardId]/project/[id]/file/[fileId].js
import { createServerSupabase } from "utils/supabaseServer";

export default async function handler(req, res) {
  const { wardId, id: projectId, fileId } = req.query;
  ...
  if (req.method === "DELETE") {
    ...
    // Delete DB row
    const { error } = await supabase.from("project_images").delete().eq("id", fileId);
