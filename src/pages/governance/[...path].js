  ...
  return <div className="flex min-h-dvh w-full flex-col">
    ...
    <main className="min-h-0 flex-1 p-3 sm:p-4">
      {view === "organization" ? (
        ...
        <div className="space-y-2"><div className="flex justify-end"><button type="button" className="text-xs text-muted-foreground hover:text-foreground" onClick={() => router.push({ pathname: router.pathname, query: { ...router.query, view: "organization" } }, undefined, { shallow: true })}>View organization →</button></div><GovernanceFamilyTree records={treeRecords} selectedId={selectedId} initialExpandedIds={lineage.map((item) => item.id)} onSelect={selectEntity} canEdit={canEdit} onAddChild={(entity) => openRelation("add-child", entity)} onAddParent={(entity) => openRelation("add-parent", entity)} onChangeParent={(entity) => openRelation("change-parent", entity)} className="min-h-[calc(100vh-7rem)]" /></div>
      )}
    </main>
    <GovernanceEntityModal open={modalOpen} onOpenChange={setModalOpen} entity={governance} parent={governance.parent_id ? byId.get(governance.parent_id) || null : null} childEntities={family.filter((entity) => entity.parent_id === governance.id)} canEdit={canEdit} onSelect={selectEntity} onSaved={handleChanged} onDeleted={handleChanged} onAddChild={(entity) => openRelation("add-child", entity)} onAddParent={(entity) => openRelation("add-parent", entity)} onChangeParent={(entity) => openRelation("change-parent", entity)} onEditLeadership={canEdit ? () => openLeadership() : null} categories={categories} />
