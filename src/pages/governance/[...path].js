...
async function getGovernanceBySlug(slug) {
  ...
  if (!result || result.error) {
    throw result?.error || new Error("Unable to load governance entity");
  }
...
export default function GovernanceRecordPage() {
  ...
  const { data: family = [], isLoading: familyLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!result || result.error) {
        throw result?.error || new Error("Unable to load governance tree");
      }
  ...
  const byId = useMemo(
    () => new Map(family.map((item) => [item.id, item])),
    [family],
  );
  ...
  const treeRecords = useMemo(() => {
    ...
    const parentById = new Map(
      family.map((entity) => [entity.id, entity.parent_id || null]),
    );
  ...
  const handleChanged = async () => {
    await Promise.all([
      ...
      queryClient.invalidateQueries({
        queryKey: ["governance-organization", governance?.id],
      }),
    ]);
    if (slug) {
  ...
  const relationCandidates = family.filter(
    (item) => item.id !== relationSource?.id,
  );
  ...
  if (governanceQuery.error || !governance) {
    return (
      <div className="flex min-h-dvh w-full flex-col">
        ...
        />
        <main className="flex flex-1 items-center justify-center text-sm">
  ...
  return (
    <div className="flex min-h-dvh w-full flex-col">
      ...
      <main className="min-h-0 flex-1 p-3 sm:p-4">
        {view === "organization" ? (
          <GovernanceOrganizationTree
            ...
            onSelect={(record) => {
              const target = record.position_governance_id
                ? byId.get(record.position_governance_id)
                : record.person_governance_id
                  ? byId.get(record.person_governance_id)
                  : null;
                  ...
                  router.push(
                    {
                      ...
                      query: { ...router.query, view: "organization" },
                    },
                    undefined,
                    { shallow: true },
                  )
      ...
      <GovernanceEntityModal
        ...
        entity={governance}
        parent={
          governance.parent_id
            ? byId.get(governance.parent_id) || null
            : null
        }
        childEntities={family.filter(
          (entity) => entity.parent_id === governance.id,
        )}
        canEdit={canEdit}
        onSelect={selectEntity}
