...
export default function GovernanceRecordPage() {
  ...
  const { data: family = [], isLoading: familyLoading } = useQuery({
    ...
    queryFn: async () => {
      const [directoryResult, geographyResult] = await Promise.all([
        supabase.rpc("get_governance_directory", { p_search: null, p_parent_id: null, p_entity_type: null, p_limit: 500, p_include_all: true }),
      ...
      return (directoryResult.data || []).map((entity) => ({ ...entity, image_url: entity.image_url || entity.metadata?.image_url || null, geography_id: geographyById.get(entity.id) ?? entity.geography_id ?? null }));
    },
  });
  ...
  const openRelation = (mode, entity) => { if (!entity?.id) return; setModalOpen(false); setModalEntity(null); setRelationMode(mode); setRelationSource(entity); setRelationOpen(true); };
  const openGeography = (entity) => { if (!entity?.id) return; setModalOpen(false); setModalEntity(null); setModalEditMode(false); setGeographyEntity(entity); setGeographyOpen(true); };
  ...
  const removeEntityGeography = async (entity) => {
    if (!entity?.id) return;
    try { await removeGeography({ governanceId: entity.id }); await handleChanged(); const { toast } = await import("sonner"); toast.success("Geography removed"); }
    catch (error) { const { toast } = await import("sonner"); toast.error(error?.message || "Unable to remove geography"); }
  };
