    ...
    async function loadApplication() {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          ...
          reviewed_at,
          created_at,
          space:space_id (id, name, slug, logo_url)
        `)
        .eq("id", id)
            ...
            <CardContent className="p-6 sm:p-8">
              <div className="flex items-center gap-4">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl border bg-muted">
                  {space.logo_url ? (
                    <img src={space.logo_url} alt={`${space.name} logo`} className="h-full w-full object-cover" />
                  ) : (
            ...
            <CardContent className="space-y-6">
              ...
              {application.status === "rejected" && application.admin_notes && (
                <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4">
                  ...
                  <p className="whitespace-pre-wrap text-sm leading-6">{application.admin_notes}</p>
                </div>
              )}
    ...
    return (
      <Alert className="border-green-500/50 bg-green-500/10 text-green-700 dark:text-green-400">
        ...
        <AlertDescription className="text-green-700/90 dark:text-green-400/90">
          Your membership application has been approved. You can now participate in this Space.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <Alert className="border-amber-500/50 bg-amber-500/10 text-amber-700 dark:text-amber-400">
      ...
      <AlertDescription className="text-amber-700/90 dark:text-amber-400/90">
        Your application has been submitted and is waiting for review by the Space administrators.
      </AlertDescription>
    </Alert>
  );
}
