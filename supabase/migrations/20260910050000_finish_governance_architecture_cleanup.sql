...
-- Replace the legacy/v2 directory API with one canonical function.
...
-- Validity is independent from current status. An active entity may have a known future end date.
...
-- Governance hierarchy mutations now operate directly on governance.
...
-- Optional bridge from canonical person entities to existing Citizen Action profiles.
...
-- Include profile-backed avatar data in organization reads.
...
returns table(
  id uuid, governance_id uuid, parent_id uuid,
  position_name text, position_governance_id uuid,
  person_name text, person_governance_id uuid,
  person_profile_user_id uuid, person_avatar_url text,
  is_vacant boolean, is_primary boolean,
  started_at timestamptz, ended_at timestamptz
)
language sql stable set search_path = public, pg_catalog
as $$
  select o.id,o.governance_id,o.reports_to_id,o.position_name,o.position_governance_id,
         o.person_name,o.person_governance_id,pg.profile_user_id,coalesce(pr.avatar_url,pg.image_url),
         o.is_vacant,o.is_primary,o.started_at,o.ended_at
  from public.organization o
  left join public.governance pg on pg.id = o.person_governance_id and pg.entity_type = 'person'
  left join public.profile pr on pr.user_id = pg.profile_user_id
...
returns table(
  id uuid, person_name text, person_governance_id uuid,
  person_profile_user_id uuid, person_avatar_url text,
  position_name text, position_governance_id uuid,
  started_at timestamptz, ended_at timestamptz, is_vacant boolean
)
language sql stable set search_path = public, pg_catalog
as $$
  select o.id,o.person_name,o.person_governance_id,pg.profile_user_id,coalesce(pr.avatar_url,pg.image_url),
         o.position_name,o.position_governance_id,o.started_at,o.ended_at,o.is_vacant
  from public.organization o
  left join public.governance pg on pg.id = o.person_governance_id and pg.entity_type = 'person'
  left join public.profile pr on pr.user_id = pg.profile_user_id
